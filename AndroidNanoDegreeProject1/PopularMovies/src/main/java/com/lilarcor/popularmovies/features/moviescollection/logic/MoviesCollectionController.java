...
public final class MoviesCollectionController implements EventBusSubscriber {
    ...
    private void configureScreen() {
        ...
        int screenWidth = mDeviceInfoProvider.getCurrentWindowWidth();
        int columns = screenWidth / baseLinePosterWidth;
        float spanWidth = baseLinePosterWidth + ((screenWidth - columns * baseLinePosterWidth) / columns);
    ...
    public void onPopularMoviesRequestFailedEvent(@NonNull PopularMoviesRequestFailedEvent event) {
        int requestPage = event.getRequestPage();

        // We only care about this if our filter is the same as the event's filter.
        if(requestPage != BaseMoviesRequestEvent.REQUEST_PAGE_NONE && mFilter == MoviesCollectionFilter.Popular) {
            mDelegate.hideSwipeIndicator();

            // If the failed request page was the first one then the user
            // either had no data to start with or performed a swipe to
            // refresh that failed.
            if(event.getRequestPage() == 1) {
    ...
    public void onTopRatedMoviesRequestFailedEvent(@NonNull TopRatedMoviesRequestFailedEvent event) {
        ...
        // We only care about this if our filter is the same as the event's filter.
        if(requestPage != BaseMoviesRequestEvent.REQUEST_PAGE_NONE && mFilter == MoviesCollectionFilter.TopRated) {
            mDelegate.hideSwipeIndicator();
